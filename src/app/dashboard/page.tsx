...
useEffect(() => {
  const fetchKriteria = async () => {
    try {
      ...
      data.forEach((k: Kriteria) => {
        // Buat key yang konsisten (misal: "harga")
        const key = (k.nama ?? k.nama_kriteria ?? "").toLowerCase().replace(/\s+/g, "_");
        
        // Simpan sebagai angka bulat (20) walaupun di DB (0.2)
        const nilaiInput = k.bobot <= 1 ? k.bobot * 100 : k.bobot;
        initialBobot[key] = nilaiInput;
      });
  ...
  const totalBobot =
  Object.keys(bobotSementara).length > 0
    ? Object.values(bobotSementara).reduce((s, v) => s + Number(v || 0), 0)
    : kriteriaList.reduce((s, k) => s + Number(k.bobot || 0), 0);
